...
space. After separation of the the exchange and correlation parts the exchange
...
\ac{LDA} describes the inhomogeneus electron density by deviding it up into
small homogeneus volumes. An improvement over \ac{LDA} can be made if the
homogeneus electrondensity is expanded in a Taylor series. Truncating after the
...
exchange-correlation hole functionit gives worse results that \ac{LDA}.

A break-through for theoretical chemistry has been achieved with the
introduction of the \ac{GGA}. It uses the \ac{GEA} hole functions and taylors
...
In the third generation D3 dispersion correction, that has been used in this
work, the calculation of $E_\text{disp}$ involves solving a two- and three-body
term. The two-body term $E^{(2)}$ is more important and is only a function of
the distance between two nuclei $r_{AB}$.
...
dispersion coefficient is calculated by averaging over the dipol
...
$C_6°^{AB}$ can be used to calculate $C_8^{AB}$ and $C_9^{AB}$, which shows up
...
The name comes from the limit of the damping function wich approaches zero with
$r_{AB}$ going to infinity. Alternatively, the Becke-Johnson damping function
can be used, which approaches a constant value with $r_AB\to\infty$.
