...
section~\ref{sec:PracticalImplementationForPotentialsDependingOnPairDistances} the gradient can be expressed as in equation~\eqref{eqn:gradient}.
%
\begin{align}
...
the redundant part of equation~\eqref{eqn:gradient} is shown in
listing~\ref{code:energy}.
%
...
has to be defined, containing the methods to calculate energy, gradient and
Hessian values. Additionally, a function that reads parameters that define the
respective potential form has to be provided. By design, the function needs to
return a pointer to an instantiation of the class on heap memory. In case of the
...
switch (status) {
  case 1: //case 1 is the LJ potential
