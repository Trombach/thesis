...
spectra of the three golden dual fullerene isomers of \ce{Au17^-}.
%
\begin{figure}[htb]\centering
	...
	\caption{Comparison of simulated photoelectron spectra of the three dual fullerene isomers of \ce{Au17^-} with (2c) and without spin-orbit coupling.} 
...
\begin{figure}[htb]
    ...
    \caption{Simulated photoelectron spectra for the negatively charged hollow
        gold clusters (shifted to the experimental threshold energy).
        \protect\subref{subfig:photo1} The two possible dual fullerene isomers
        of \ce{Au16^-}. The green curve shows a combination of the
        $D_\mathrm{2}$ and $T_\mathrm{d}$ spectra with a ratio of 1:1.;
        \protect\subref{subfig:photo2} The three possible dual fullerene isomers
        of \ce{Au17^-}; \protect\subref{subfig:photo3} The six possible dual
        fullerene isomers of
        \ce{Au18^-} (shifted to the experimental threshold energy).}
...
dual fullerene isomers for \ce{Au17^-}. The spectra have been shifted according
to the vertical ionisation potential of the negatively charged clusters. The
...
vertical ionisation potential first and subsequently shifted by 0.25~eV to
...
\chapter[From Sticky-Hard-Sphere to Lennard-Jones-Type Clusters]{From
    ...
    and is reprinted with permission from the publisher \textcopyright 2018
...
\end{equation}
%
Using the Gregory-Newton argument results in $f(N)=6N$, however, a tighter upper
...
certain size $N$ differs substantially between the two potentials. For $N=13$
...
clusters\autocite{Doye_Evolutionpotentialenergy_1999}. However, this is a known
...
\begin{table}[htbp]\centering
    \resizebox{\linewidth}{!}{%
        ...
        starting from the set $\mathcal{M}_{\mathrm{SHS}}(N,N_c)$ for a
...
$|\mathcal{M}_\mathrm{SHS\to LJ}|$ only misses out on a small portion of
minima obtained from the more exhaustive basin-hopping approach,
i.e.~$|\mathcal{M}_\mathrm{SHS\to LJ}|\approx|\mathcal{M}_\mathrm{LJ}|$.
...
recently given value of $\alpha=1.04$ by Forman and Cameron
...
\begin{figure}[htb]
    ...
    \caption{Histogram of the energies (bin size $\Delta E=0.1$) of
...
\begin{table}[htb]\centering
    \caption{Range $[E_0=0,E_\text{max}]$ of the energy spectrum of all \acs{LJ}
    ...
    respective global minimum (in reduced units).}
    \label{tab:energies}
...
as initial states for energy minimisation have been characterised. The number of
...
equilibrium distance have been arbitrarily set to 1.
...
via vertices (cuboctahedron), while in the \ac{hcp} case the square faces come
...
section~\ref{sec:addinga14thsphere}.} out of all 895,478
...
these clusters is required to enter the second coordination shell because of
...
\begin{figure}
    ...
    structures from
    \citeauthor{Holmes-Cerfon_EnumeratingRigidSphere_2016}\autocite{Holmes-Cerfon_EnumeratingRigidSphere_2016}.
    The width of the bars is $0.01$~$r_e$.}
...
with maximum edge count of 24 corresponding to the sphere packings of the
