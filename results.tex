...
interest in this field resurged over the recent
...
carbon fullerene with a gold atom. The result is a triangulated surface.
...
which is $2$ for genus $g=2$ surfaces as in convex polyhedra. As shown in
...
for gold structures.\autocite{Assadollahzadeh_systematicsearchminimum_2009}
...
with each other.\autocite{Kroto_stabilityfullerenesCn_1987} It is hitherto unknown if there is and equivalent ``isolated
vertex of degree five rule'' for dual fullerene structures.
...
The original Goldberg-Coxeter transformation was carried out on the \ce{C20}
...
transformation) or $l=0$  (halma transformation). Some important transformations are for
example
...
the initial point group. In case of the gold clusters the same transformations
results in the respective dual representations, i.e.
...
clusters.\autocite{Johansson_Au3224CaratGolden_2004} The new vertex count of $GC_{k,l}[\ce{Au_{|N_d|}}]$ is
%
\begin{align}
...
icosahedral shells. Each individual shell $m$ is made up of
%
\begin{align}
...
the icosahedron including the spheres marked in red. There is exactly one sphere
more on the edges than there are shells, thus $m=|N_\text{edge}|-1$. The halma
pattern of a $GC_{k,0}$ transformation is clearly visible on the faces of the
icosahedron, and it turns out the icosahedral shells are in fact related to the
smallest fullerene \ce{C20} by such a transformation and a subsequent
...
with radius $R_\mathrm{MDS}$. In other words, a sphere that approximates the
position of the vertices best is looked for. A measure for distortion from
spherical symmetry through the
...
The neutral gold clusters and their propertiy convergence towards the bulk has
...
\resizebox{\linewidth}{!}{%
    ...
    \caption{Number of distinct local minima $|\mathcal{M}_\mathrm{SHS}|$ for
        ...
        (6,12)-LJ potential. The overall number of unique minima
...
\subsection{Rigid Gregory-Newton Clusters and Corresponding Graphs}
