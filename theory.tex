...
A sufficiently accurate description of atoms and molecules is given by the
Schr\"odinger equation. The following chapters introduces the fundamentals of
...
the \textit{wave function} $\Psi$ at the centre of
...
the wave function which results in a solution for the total energy $E$ of the
system. 
...
density} which is used to normalise it such that the probability of finding an
electron anywhere in space is one. This interpretation is also referred to as
...
$g_{ij}=\frac{1}{|\mathbf{r}_i-\mathbf{r}_j|}$. Its action on the part of the
...
Solving the canonical Hartree-Fock equations numerically for larger systems is
