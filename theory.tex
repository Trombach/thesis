...
contains the graph $H$, $H$ is called a subgraph of $G$. This requires the
...
bridging network, but moreso with how to traverse over it. In graph theoretical
terms the desired solution is called a closed trail, which is a a special case
...
closed tail traversing all edges is also called Eulerian in honor of of Euler's
...
\end{figure}
%
The maximally connected subgraphs of a graph are called its components. In
...
induced subgraphs. If we remove a list of vertices $T$ from a graph $G$ the
vertex-induced subgraph $H$ with the vertex set $V(H)=V(G)-T$ is obtained. All
edges incident to one of the removed edges need to be removed as well. Removing
the edge set $S$ results in the edge-induced subgraph $I$ with the edge set
...
rest of the polyhedron can sit inside the streched face
...
The beginning of the 20th century was a very important time for modern
...
the following sections the concept of inverse space and its application to
...
The geometry of a crystal can be described in real and inverse space. In real
space the crystal lattice can be produce by the lattice vector
...
this problem the electronic structure is usually solved in inverse space.
Instead of solving an infinitely large problem, inverse space allows to
transform the system into infinitely many sub-systems, with each sub-system
having a finite solution. 

Real and inverse space are connected by a Fourier transformation, which leads
to the inverse lattice parameters $\mathbf{b}_i$, that relate to the real space
...
\end{align}
%
Translation in inverse space is therefore defined as:
%
\begin{align}
...
The lattice vector in inverse space $\mathbf{K}$ is related to $\mathbf{R}$ via
$\exp\{i\mathbf{KR}\}=1$ and the corresponding basis vectors in real and
inverse space must satisfy the condition
$\mathbf{a}_i\mathbf{b}_i=2\pi\delta_{ij}$.

In inverse space the primitive unit cell is also called the Brillouin zone and
...
system, the same must apply to the interatomic potential $V$.
%
\begin{align}
...
Ann \ac{ECP} has to be based on an all-electron calculation that describes the
...
For example, the Stuttgardt-K\"oln pseudo-potentials use Gaussian functions in
...
cartesian coordinates $\Delta\mathbf{x}$.\autocite{Peng_Usingredundantinternal_1996}
%
\begin{align}
...
\end{align}
%
The first derivative of the Wilson matrix $\mathbf{W}'$ is calculated analytically.
...
\chapter{Potentials for Nucleation Problems}
