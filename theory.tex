...
Alternatively, the incidence matrix $M$ is an $n \cross m$ matrix where each
...
An important concept utilised in the last chapter of this thesis is that of
...
to equation~\eqref{eqn:EigenVectorFollowingStepsize} such that the step size
...
via equation~\eqref{eqn:EigenVectorFollowingStep}. This step will then move
...
equation~\eqref{eqn:EigenVectorFollowingStep} for $\lambda=0$. This procedure can
...
equation~\eqref{eqn:VirialEquationOfState}.} to be expressed as functions of
intermolecular potentials. If all virial coefficients are 0,
equation~\eqref{eqn:VirialEquationOfState} is equal to the ideal gas equation,
...
The partition function $Z_N$ can be used to derive various thermodynamic quantities
...
By reversing the definition of the U-functions the Boltzmann factors can be
...
\end{align}
%
Now equation~\eqref{eqn:PartitionFunction} can be expressed in terms of the
...
similar form to equation~\eqref{eqn:VirialEquationOfState}.
%
\begin{align}
...
\eqref{eqn:VirialEquationOfStateClusterIntegrals} it becomes clear that the
...
The repulsive part is more complicated, as it can not be derived directly form
...
in terms of equation~\eqref{eqn:LJgeneral_theory}, equation~\eqref{eqn:SHSpotential}
...
equation~\eqref{eqn:SHSpotential}. As the \ac{SHS} is not a continuous function,
...
where a contact state is represented by a matrix element of
$\mathcal{A}_{ij}=1$ and every other position by
$\mathcal{A}_{ij}=0$.\autocite{Arkus_DerivingFiniteSphere_2011} The problem of
...
to their suitability for a stable cluster structure. A lot of the possible
adjacency matrices can be rejected immediately, because they represent a already
found structure with a different particle labelling. This particle labelling
degeneracy comes from the fact that the spheres are all equal and therefore
swapping two rows or columns in the adjacency matrix won't change the underlying
cluster structure. If two adjacency matrices are corresponding to the same
structure they are said to be \textit{isomorphic} (see
chapter~\ref{sec:DefinitionGraph}).
...
that the contact number needs to be $N_c\geq 3N-6$ for a structure to be
...
which need to be determined algebraically. For this, the distance matrix
$\mathcal{D}$ needs to be constructed from the adjacency matrix. Based on the
adjacency matrix $3N-6$ equations (and $N(N-1)/2 - (3N-6)$ inequalities) can be
defined.
%
\begin{align}
    \mathcal{A}_{ij}=1\rightarrow r_{ij}=2r\\
    \mathcal{A}_{ij}=0\rightarrow r_{ij}>2r
\end{align}
%
...
rule solves for the mapping $\mathcal{A}_{ij}\to \mathcal{D}_{ij}$. These rules
...
radius $\frac{\sqrt{3}}{2}R$. Therefore, each matrix element $\mathcal{A}_{ij}$
