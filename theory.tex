...
scope of this thesis. I not mentioned otherwise the chapter is mainly based on
a book by
...
terms the desired solution is called a trail, which is a a special case a walk,
where no edge can be repeated (i.e. no bridge can be crossed twice). A walk
describes a way to traverse over a graph by defining a list of vertices and
edges $v_0,e_1,v_1,\dots,e_k,v_k$ where the endpoints for each edge $e_i$ have
to be $v_{i-1}$ and $v_i$ ($1\leq i\leq k$). One possible trail (excluding one
...
\begin{figure}[htb]
    ...
    \subfloat[A,C-path\label{subfig:acpath}]{
        \begin{tikzpicture}
            ...
            \draw[red,thick] (A) -- (B);
            \draw[red,thick] (A) to [out=100,in=170] (C);    
            \draw[red,thick] (C) to [out=260,in=10] (A);    
            \draw[red,thick] (D) to [out=100,in=350] (A);    
            \draw[blue,directed,thick] (A) to [out=260,in=190] (D);    
            \draw[blue,directed,thick] (D) -- (B);    
            \draw[blue,directed,thick] (B) -- (C);    
        \end{tikzpicture}
...
$u,v$-path for $G$ to be connected.
