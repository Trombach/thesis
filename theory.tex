...
derive parameters for $\lambda_n$ and $\lambda_m$ based on experimental
results.\autocite{Jones_DeterminationMolecularFields_1924}

Equation~\ref{eqn:LJgeneral} can be redefined in terms of parameters for the
...
the \ac{LJ} potential is arbitrary and lacks flexibility. Any effect that
scales differently from $r^{-12}$ or $r^{-6}$ can not be described accurately.
Therefore, it seems almost natural to extend the \ac{LJ} potential by a sum
over different $r^{-i}$ terms weighted by coefficients $c_i$.
%
\begin{align}
...
study investigated the Xenon dimer and it was decided to include exponents up
to $i=18$ and also some odd numbered
ones.\autocite{Jerabek_relativisticcoupledclusterinteraction_2017} In both
cases the coefficients $c_i$ are determined by a fitting procedure to very
accurate dissociation curves of the respective dimer molecules calculated by
...
and as a benchmark for global optimisation methods. From the derivation of the
\ac{LJ} potential as a interaction potential to get analytical solutions for
the second virial coefficient it should be clear, that it is a rather crude
...
predictions especially in the case of rare gas
...
experimental results.\autocite{Kakar_SizedependentKedgeEXAFS_1997}

Within the Born-Oppenheimer approximation
(section~\ref{sec:bornoppenheimerapproximation}) the movement of the nuclei can
be approximated to be described by a \ac{PES}, also called the energy
landscape. The \ac{PES} is a multidimensional function\footnote{In three
dimensions the number of variables is equal to $3N$.} and is entirely defined
b the inter-atomic potential. 
