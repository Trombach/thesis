...
Jensen\autocite{Jensen_IntroductionComputationalChemistry_2006}, Szabo and
...
%
\begin{align}
    \mathbf{g}(\mathbf{x^*})&=0\label{eqn:firstordernecessary}\\
    \mathbf{s^\text{T}}\mathbf{G}(\mathbf{x^*})\mathbf{s}&\geq 0.\label{eqn:secondordernecessary}
\end{align}
...
$\mathbf{s^\text{T}}\mathbf{G}(\mathbf{x^*})\mathbf{s}>
...
fulfil the first condition $\mathbf{g}(\mathbf{x^*})=0$. As those points can
...
$\mathbf{G}(\mathbf{x})$ are used to verify local minimisers, which have to be
positive.
...
%
\begin{equation}
    \frac{||\Delta\mathbf{x}^{(k+1)}||}{||\Delta\mathbf{x}^{(k)}||^{p}}\leq a, \,a>0
\end{equation}
%
...
$k$th iteration requires (1) to determine the search direction
$\mathbf{s}^{(k)}$, (2) minimise $f(\mathbf{x}^{(k)}+\alpha\mathbf{s}^{(k)})$
with respect to $\alpha$ and (3) to set the new iterate
$\mathbf{x}^{(k+1)}=\mathbf{x}^{(k)}+\alpha^{(k)}\mathbf{s}^{(k)}$. Step (1)
depends on the chosen method, while step (2) is independent of the chosen
method and corresponds to the line search step. The latter is solved by
sampling $f(\mathbf{x})$ along the line $\mathbf{s}^{(k)}$.



\section{Optimisation Schemes}
\label{sec:OptimisationSchemes}
