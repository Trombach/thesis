...
A relation between the number of vertices $|V|$, regions $|F|$ and edges $|E|$ in a
planar graph is given by Euler's polyhedral formula.
%
\begin{align}
    |V|-|E|+|F|=2\label{eqn:Euler}
\end{align}
%
...
vertices in a graph, because exchanging $V$ and $F$ doesn't change its result.
This property is known as \textit{duality}. The geometric dual of a graph $G'$
has $|V|$ faces and $|F|$ vertices, while the edge count remains unchanged. It
...
\end{align}
%
Mappings have to created vertex by vertex, so initially all mappings start as
...
first order stationary points. Knowledge about the nature and location of first- and higher-order
stationary points is crucial for a thorough understanding of the topology of the underlying
...
as a side effect. Such global optimisation problems belong the the complexity class of
``NP-complete'' problems, meaning no algorithm is currently known that is guaranteed to
...
There are many optimization algorithms for search for stationary points on a PES all with their advantages and disadvantages. Only few were mentioned here used in cluster optimizations.
...
the following section. It is clear that convergence of this expansion is realized
if the  virial coefficients are small compared to the volume $v$.
...
coupled-cluster theory. Such a potential has the advantage that for the solid state
the cohesive energy can be expressed analytically in terms of lattice sums.\autocite{Schwerdtfeger_ExtensionLennardJonespotential_2006}
