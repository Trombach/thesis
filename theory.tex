...
be possible in future, but currently too costly with the algorithms available.  
...
which implies the electrons do not interact directly with each other, but each
electron is moving in a mean field created by all the other electrons. The Hartree-Fock
energy $E_\text{HF}$ is therefore never exact for a multi-electron system even in the infinite basis set
limit. The difference to the exact energy $E_0$ was first named electron correlation
...
of the total electronic energy, it is an important contribution in molecular systems
when energy differences are involved. 
A multitude of methods has therefore been developed to treat electron correlation more
...
(or its multi-reference analogs) as a starting point, \ac{DFT} is in principle a wave function free method. It
establishes a connection between the energy of the system and the (one-particle) electron
density $\rho({\mathbf r})$ instead of the complex wave function dependent on the coordinates
of all particles. The electron density is an observable
and a positive real function, which makes \ac{DFT} easier to grasp than wave
...
finding one of the $N$ electrons in the volume $\dd \mathbf{r}_1$. 
If we include spin we arrive at a similar expression for the one-particle spin density. 
%All other $(N-1)$ electrons occupy any volume elements, but have opposite spin. 
As electrons are indistinguishable the probability of finding
...
\end{align}
%
The electron density resulting from the KS orbitals is required to be
equal to the (in principle exact) ground state density of the real system.
%
\begin{align}
...
than $r_e$. First attempts at deriving correct long range behaviour have been
made by considering two hydrogen
atoms.\autocite{Wang_gegenseitigeEinwirkungzweier_1927} The attractive force
was shown to scale with $r^{-7}$, which could not be confirmed by other
investigations. Works by other authors showed the behaviour to be on the order
of
