...
where a contact state is represented by a matrix element of $\mathcal{A}_{ij}=1$
...
rejected immediately, because they represent a already found structure with a
different particle labelling. This particle labelling degeneracy comes from the
...
adjacency matrices are corresponding to the same structure they are said to be
...
contacts. This is likely to be due to the fact that in this study a modification
of the exact enumeration method was used, which follows on-dimensional
transition paths created from breaking a random contact in an already found
cluster. An overview of these results can be found in table~\ref{tab:comp}
