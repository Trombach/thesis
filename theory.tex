...
set} $N(G)$ and an \textit{edge set} $E(G)$ that connects pairs of vertices via
...
polyhedron $P$ if $G$ is planar and cubic. A cubic graph is a graph with every
vertex having at least degree three. This relation can be illustrated by the
...
vertices in a graph, because exchanging $N$ and $F$ doesn't change its result.
