...
terms the solution is called a trail, which is a a special case a walk, where
no edge can be repeated. A walk describes a way to traverse over a graph by
defining a list of vertices and edges $v_0,e_1,v_1,\dots,e_k,v_k$ where the
endpoints for each edge $e_i$ have to be $v_{i-1}$ and $v_i$ ($1\leq i\leq k$).
%
\begin{figure}
    \centering
    \begin{tikzpicture}
        ...
        \draw[red,thick] (B) -- (C);    
    \end{tikzpicture}
    \caption{Example of one possible trail over the bridges of K\"onigsberg.
    Including the red edge requires reusing some bridges and is therefore a
    walk.}
